...
The conversion from year (\texttt{YEAR}) and day of the year (\texttt{DOY}) is perform by \code{as.Date<-} method. It stores dates as \code{Date} objects in a new column named \texttt{date\_YEAR\_DOY}. 
...
wthFiles <- list.files(path=wthPath, pattern="WTH", full.names=TRUE)
...
<<echo=TRUE,print=FALSE,fig=FALSE>>=
plot(TMAX ~ as.Date(as.character(DATE),format="%y%j"), wthSeries)
@  
...
We will read the experimental file \texttt{SANT7001.MZX} and we will replace the fertilization doses. Because this file will be replaced, we make a copy on our working directory.

<<eval=TRUE,echo=TRUE>>=
santfile <- system.file("extdata","SANT7001.MZX",package="Dasst")
file.copy(santfile, "SANT7001.MZX")
sant <- read.dssat("SANT7001.MZX")
sant[9]
