...
##' Gather tables from object of class Dasst.
##'
##' \code{gatherTables} gathers the result of performing
##' a certain operation over selected tables of an object
...
##'
gatherTables <- function(object, indices, coCol, opCol, operation, ...){
...
### check function argument type
  ...
  if(length(indices) == 1){
    index <- indices[1]
    return(object[index][,c(coCol,opCol)])
    ...
    if(all(nzchar(coCol))){
      index <- indices[1]
      for(col in coCol){
        redData[[col]] <- object[index][[col]][1:minLen]
      }
    }
    for(col in opCol){
    ...
    cat("Error: indices must be integers in the range from 1 to ", length(object), "\n")
    return(NULL)
